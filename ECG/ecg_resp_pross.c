...
const float IIR_High_A[3]={
        1,   -1.978675008,   0.9788999557 
	       //1,   -1.994202495,   0.9945111275 
};


/*****************************33Hz_IIR__Low_Filter 3order****************************/
 const float IIR_LOW_B[3]={1,              2,              1 };
 const float IIR_LOW_A[3]={   1,  -0.5494753122,   0.2254591584    };
...
/* w[0] =current value,  w[1] = previous value **/
 w[0] = *x + alpha*w[1];
 *filtered_ = w[0] - w[1];
 w[1] = w[0];
}
...
float ecg_filters_(float samples,float *w0,float *w1,float *w2)
{
	  ...
	  float CurrAqsSample[1] = {0};
	
       /*DC removal*/
			 dcRemoval(&samples,0.90,CurrAqsSample);
	
	
		  w0[0]=IIR_50Notch_A[0]**CurrAqsSample-IIR_50Notch_A[1]*w0[1]-IIR_50Notch_A[2]*w0[2];
...
//			y2=Gain*(IIR_LOW_B[0]*w2[0]+IIR_LOW_B[1]*w2[1]+IIR_LOW_B[2]*w2[2]);
		  ...
		  w2[0]=IIR_LOW_A[0]*y0-IIR_LOW_A[1]*w2[1]-IIR_LOW_A[2]*w2[2];
			y2=Gain*(IIR_LOW_B[0]*w2[0]+IIR_LOW_B[1]*w2[1]+IIR_LOW_B[2]*w2[2]);
...
void ECG_ProcessCurrSample_(double *Curr, double *FilteredOut)
{
	     double CurrAqsSample[1] = {0};
	
       /*DC removal*/
			// dcRemoval(Curr,0.95,CurrAqsSample);
			 
       w0[0]=IIR_50Notch_A[0]*(*Curr)-IIR_50Notch_A[1]*w0[1]-IIR_50Notch_A[2]*w0[2];
			y0=IIR_50Notch_B[0]*w0[0]+IIR_50Notch_B[1]*w0[1]+IIR_50Notch_B[2]*w0[2];
  ...
  if ( TRUE == threshold_crossed  )
  {
    ...
    */
			...
			
    {
			...
			//Display the heart rate and the QRS etc
			HR_HRAvg_disp_flag = 1;
