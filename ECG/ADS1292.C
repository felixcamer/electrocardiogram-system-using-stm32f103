...
{		
	 ...
	 //SPI_InitStructure.SPI_CPOL = SPI_CPOL_High;
  	...
  	GPIO_EXTILineConfig(GPIO_PortSourceGPIOA,GPIO_PinSource4);//选择管脚
  	...
  	NVIC_Init(&NVIC_InitStructure); 
		...
		ADS1292_PowerOnInit();//上电复位，进入待机模式	
				...
				while(Set_ADS1292_Collect(0))//0 正常采集 //1 1mV 1Hz内部侧试信号
		{	
			printf("1292 reg err\r\n");
...
u8 ADS1292_Read_Data(u8 *data)//72M时钟下函数耗时大约10us  8M时钟下 函数耗时 100us
{		
		u8 i;	
...
void ADS1292_SET_REGBUFF(void)
{
	...
	ADS1292_REG[LOFF] =	0xF4;//[7:5]	设置导联脱落比较器阈值 [4]	必须为1 		[3:2] 导联脱落电流幅值		[1]	必须为0	[0]	导联脱落检测方式 0 DC 1 AC 
	...
	ADS1292_REG[RLD_SENS] = 0X00; //00ab cdef	[7:6] 00 PGA斩波频率	fMOD/16 
...
u8 ADS1292_Single_Test(void) //注意1292R开了呼吸解调，会对通道一的内部测试信号波形造成影响，这里只参考通道2即可，1292不受影响
{
		...
		//Ads1292_Ch2set.MUX=MUX_VDD_signal;//测试信号输入	
...
void stop_ads1292(){
	...
	ADS1292_Send_CMD(STOP);	//发送开始数据转换（等效于拉高START引脚）	
	delay_ms(1);	
